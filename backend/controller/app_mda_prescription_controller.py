...
@router.get("/get-all", response_model=list[PrescriptionResponse])
def get_all(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return prescription_service.get_all(db, skip=skip, limit=limit)


@router.get("/get-by-guid/{guid}", response_model=PrescriptionResponse)
...
@router.post("/create", response_model=PrescriptionResponse, status_code=201)
def create(payload: PrescriptionCreate, db: Session = Depends(get_db)):
    return prescription_service.create(db, payload.model_dump())


@router.put("/update/{guid}", response_model=PrescriptionResponse)
