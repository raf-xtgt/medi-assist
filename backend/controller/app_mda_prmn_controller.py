...
@router.get("/get-all", response_model=list[PrmnResponse])
def get_all(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return prmn_service.get_all(db, skip=skip, limit=limit)


@router.get("/get-by-guid/{guid}", response_model=PrmnResponse)
...
@router.post("/create", response_model=PrmnResponse, status_code=201)
def create(payload: PrmnCreate, db: Session = Depends(get_db)):
    return prmn_service.create(db, payload.model_dump())


@router.put("/update/{guid}", response_model=PrmnResponse)
