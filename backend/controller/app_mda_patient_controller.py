...
@router.get("/get-all", response_model=list[PatientResponse])
def get_all(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return patient_service.get_all(db, skip=skip, limit=limit)


@router.get("/get-by-guid/{guid}", response_model=PatientResponse)
...
@router.put("/update/{guid}", response_model=PatientResponse)
